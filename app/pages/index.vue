...
// ── Draft autosave (IndexedDB → Mongo) ────────────────────────────────────
const pageId = computed(() => {
  const name = pdfFile.value?.name || htmlFile.value?.name || ''
  return name ? name.replace(/\.(pdf|html?|css)$/i, '') : 'untitled'
})
...
async function restoreDraft() {
  ...
  try {
    ...
    if (data) {
      if (data.html) htmlContent.value = data.html
      if (data.css) setCssText(data.css)
    }
  } finally {
    await nextTick()
    suppressDraft = false
